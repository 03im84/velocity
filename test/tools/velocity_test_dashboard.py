...
APP_VERSION = "0.3.2"
PHASE_LABEL = "Phase 3.1 — Pause and Resume"
...
DEFAULT_SHARED_CONFIG: dict[str, Any] = {
    ...
    "suites": {
        "All": ["test/core"],
        "DeviceBus": ["test/core/device_bus"],
        "DeviceCore": [
            "test/core/device_core",
            "test/core/device",
        ],
        "Providers": ["test/core/provider"],
        "Profiles": ["test/core/profile"],
        "Message Contracts": [
            "test/core/message_contract"
        ],
        "Debug": ["test/core/debug"],
    },
}
...
class VelocityTestDashboard:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.shared_config_path = (
            self.tools_directory / SHARED_CONFIG_NAME
        )

        self.local_config_path = (
            self.tools_directory / LOCAL_CONFIG_NAME
        )
        ...
        self.execution_status_var = tk.StringVar(
            value="IDLE"
        )

        self.summary_var = tk.StringVar(
            value="No execution plan."
        )
    ...
    def _build_browser(self, parent: ttk.Frame) -> None:
        ...
        self.test_tree.column("suite", width=110)
        self.test_tree.column("path", width=330)
    ...
    def _build_execution_panel(
        ...
        self.result_tree.heading("#0", text="Test")
        self.result_tree.heading("status", text="Status")
        self.result_tree.heading("exit", text="Exit")
        self.result_tree.heading("runs", text="Runs")
        ...
        self.result_tree.column("status", width=110)
        self.result_tree.column("exit", width=60)
        self.result_tree.column("runs", width=60)
    ...
    def _apply_configuration(self) -> None:
        ...
        godot_text = (
            environment_godot or local_godot
        )
        ...
        self.suite_var.set(preferred_suite)
    ...
    def _valid_godot_console(self) -> bool:
        return bool(
            ...
            and self.godot_console.suffix.lower()
            == ".exe"
        )
    ...
    def refresh_tests(self) -> None:
        ...
        if selected:
            previous_path = selected.resource_path
        else:
            previous_path = str(
                self.local_config.get(
                    "last_selected_test",
    ...
    def _discover_tests(self) -> list[TestScene]:
        ...
        roots = self.shared_config.get(
            "test_roots",
            ["test/core"],
        )

        includes = self.shared_config.get(
            "include_patterns",
            ["*Test.tscn", "*_test.tscn"],
        )

        excludes = self.shared_config.get(
            "exclude_patterns",
            [],
        )
        ...
        for root_text in roots:
            test_root = (
                self.project_root / str(root_text)
            ).resolve()

            if not test_root.is_dir():
                continue

            for scene_path in test_root.rglob(
                "*.tscn"
            ):
                ...
                discovered[resource] = TestScene(
                    ...
                    suite=self._suite_for(relative),
                )
    ...
    def _suite_for(self, relative: str) -> str:
        normalized = relative.replace(
            "\\",
            "/",
        ).lower()

        suites = self.shared_config.get(
            "suites",
            {},
        )
    ...
    def _refresh_suite_values(self) -> None:
        ...
        discovered = sorted(
            {item.suite for item in self.tests},
            key=str.lower,
        )
    ...
    def _selected_test(self) -> TestScene | None:
        ...
        return self.test_tree_items.get(
            selected_ids[0]
        )
        ...
        try:
            while True:
                ...
                if event_type == "line":
                    ...
                    self._complete_current_test(
                        int(value)
                    )

                elif event_type == "worker_error":
                    self._fail_current_test(
                        str(value)
                    )
    ...
    def _complete_current_test(
        ...
        parsed = self._parse_runner_summary(
            result.output
        )

        result.total_runs = parsed.get(
            "total_runs",
            0,
        )

        result.passed = parsed.get(
            "passed",
            0,
        )

        result.failed = parsed.get(
            "failed",
            0,
        )
    ...
    def _status_from_output(
        ...
        if (
            "TIMEOUT" in output
            or "ExitCode 124" in output
        ):
            return "TIMEOUT"

        if (
            "ENGINE_ERROR" in output
            or "ExitCode 126" in output
        ):
            return "ENGINE_ERROR"
    ...
    def _finish_plan(
        ...
        self._append_output(summary_output)
    ...
    def _update_button_states(self) -> None:
        ...
        self.select_godot_button.configure(
            state=normal_state
        )

        self.repeat_spinbox.configure(
            state=normal_state
        )

        self.timeout_spinbox.configure(
            state=normal_state
        )
